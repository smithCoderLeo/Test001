...
#####################################################################################
list_noise = []
with open(configdir + 'noise.txt','r') as file: #这两个处理的目的
...
RE_file2e = re.compile(r'.*?\.([eE][xX][eE]|[dD][lL][lL]|[dD][rR][vV]|'
                       ...
                       r'[cC][pP]?[pP]?$|'
                       r'[bB][aA][tT]|manifest)+.*?') #exe dll jpeg gif bmp png
...
class PEfileExtract(object):  # pe文件提取
    def __init__(self, f, fw):  #
        ...
        t2 = time.clock()
        print 'extract finished time: ' + str(t2 - t1) + ' s'  # 打印提取时间
    ...
    def initWriteFile(self, fw):  # 初始化写入的目标文件
        # t = self.file_original = open(fw + '/originalfile.exe','w')
        # self.list_file.append(t)
        t = self.file_pefile = open(fw + '/13.pefile.txt', 'w')
        self.list_file.append(t)
        num = len(self.list_pe_string) + len(self.list_pe_date) + len(self.list_pe_importDllName) + len(
            self.list_pe_importDll) + len(self.list_pe_export)
        ...
        self.list_file.append(t)
        num = len(self.list_by) + len(self.list_copyright) + len(self.list_version) + len(self.list_called) + len(
            self.list_year)
        t = self.file_otherInfo = open(fw + '/3.otherInfo_' + str(num) + '.txt', 'w')
        self.list_file.append(t)
        t = self.file_noMeaningString = open(fw + '/11.noMeaningString_' + str(len(self.list_noMeaningString)) + '.txt',
                                             'w')
        self.list_file.append(t)
    ...
    def pefileExtractFile(self, f, fw):
        ...
        try:
            ...
            # import
            try:
                for importdll in self.pe.DIRECTORY_ENTRY_IMPORT:  # 提取导入表
...
def initConfig(filename): #初始化设置文件
    list = []
    #print filename
...
def isNoise(str): #判断是否是噪声
    for line in list_noise:
        if line in str:
            return True
    return False

def isVisible(c): #判断是否可见字符
